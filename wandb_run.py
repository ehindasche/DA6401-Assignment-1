...
def train():
    ...
    nn = NeuralNetwork(input_size=784, output_size=10, beta1=0.8, beta2=0.999, optimizer=config.optimizer, init_method= config.weight_init, momentum=config.momentum, beta=config.beta, batch_size=config.batch_size)
    for _ in range(config.hidden_layers):
        nn.add_layer(config.hidden_size, activation=config.activation)
    
    nn.train(X_train, y_train, X_val, y_val, epochs=15, lr=config.learning_rate, loss_type='mse')
