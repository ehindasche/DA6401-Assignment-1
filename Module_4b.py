...
def train():
    ...
    nn = NeuralNetwork(input_size=784, output_size=10, beta1=config.beta1, beta2=config.beta2, optimizer=config.optimizer, init_method= config.weight_init, momentum=config.momentum, beta=config.beta, batch_size=config.batch_size)
    for _ in range(config.hidden_layers):
        nn.add_layer(config.hidden_size, activation=config.activation)
    
    nn.train(X_train, y_train, X_val, y_val, epochs=config.epochs, lr=config.learning_rate, loss_type=config.loss_type)
